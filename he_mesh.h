...
public:
	vertex();
	vertex(int theId, Vector3 theLoc) {
		loc = theLoc;
		id = theId;
	}
...
public:
	...
	edge(int theID, vertex * theVertex) {
		id = theID;
		vert = theVertex;
	}

	edge(int theID, vertex *theVertex, edge * thePair) {
		id = theID;
		vert = theVertex;
		pair = thePair;
	}
...
public:
	...
	//Get an updated vertex based on its old id 
	vertex* findVertexByOldID(int id) {
		for (int i = 0; i < verteces.size(); i++) {
			if (verteces[i]->oldID = id) {
				return verteces[i];
			}
		}
		return NULL;
	...
	//Get an vertex based on its position and nothing else
	vertex *findVertexByLocation(Vector3 loc) {
		for (int i = 0; i < verteces.size(); i++) {
			if (loc == verteces[i]->loc) {
				return verteces[i];
			}
		}
		return NULL;
